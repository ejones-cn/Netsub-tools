...
def update_version_file(major, minor, patch):
    ...
    content = re.sub(r'VERSION_TUPLE\s*=\s*\(\d+,\s*\d+,\s*\d+\)', f'VERSION_TUPLE = ({major}, {minor}, {patch})', content)
    
    # 更新或添加发布日期
    today = datetime.now().strftime("%Y-%m-%d")
    if f'"{new_version}"':
        content = re.sub(rf'"{new_version}":\s*"\d{{4}}-\d{{2}}-\d{{2}}"', f'"{new_version}": "{today}"', content)
    else:
        # 如果版本号不存在，在RELEASE_DATES中添加
        new_release_line = f'"{new_version}": "{today}"' + ',\n'
        content = re.sub(r'RELEASE_DATES\s*=\s*\{', 
                        f'RELEASE_DATES = {{{new_release_line}', content)
...
def update_readme_file(new_version):
    ...
    # 更新标题中的版本号
    content = re.sub(r'# IP子网切分工具 v\d+\.\d+\.\d+', f'# IP子网切分工具 v{new_version}', content)
...
def update_windows_app_file(new_version):
    ...
    # 更新app_version变量
    content = re.sub(r'self\.app_version\s*=\s*"\d+\.\d+\.\d+"', f'self.app_version = "{new_version}"', content)
